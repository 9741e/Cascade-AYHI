...
def binary_search(Alice_block: np.ndarray, Bob_block: np.ndarray, recusion: bool, bpq_bspb_exp: np.ndarray, blocks: list, start: int, end: int):
    if not recusion:
        bpq_bspb_exp[0] += 1
        if f"{start}:{end}" not in blocks:
    ...
    if(parity_check(Alice_block, Bob_block)):
        if(np.size(Bob_block) == 1):
            ...
            if not recusion:
                bpq_bspb_exp[1] += 1
                blocks.append(f"{start}:{start+math.floor(np.size(Alice_block)/2)}")
                blocks.append(f"{start+math.floor(np.size(Alice_block)/2)}:{end}")
            elif f"{start}:{start+math.floor(np.size(Alice_block)/2)}" not in blocks:
                bpq_bspb_exp[1] += 1
                bpq_bspb_exp[0] += 1
                blocks.append(f"{start}:{start+math.floor(np.size(Alice_block)/2)}")
                blocks.append(f"{start+math.floor(np.size(Alice_block)/2)}:{end}")
            if(parity_check(Alice_block[0:math.floor(np.size(Alice_block)/2)], Bob_block[0:math.floor(np.size(Bob_block)/2)])):
                binary_search(Alice_block[0:math.floor(np.size(Alice_block)/2)], Bob_block[0:math.floor(np.size(Bob_block)/2)], recusion, bpq_bspb_exp, blocks, start, start+math.floor(np.size(Alice_block)/2))
            else:    
                binary_search(Alice_block[math.floor(np.size(Alice_block)/2):], Bob_block[math.floor(np.size(Bob_block)/2):], recusion, bpq_bspb_exp, blocks, start+math.floor(np.size(Alice_block)/2), end)
...
def Cascade(A_key: np.ndarray, B_key: np.ndarray, k: list, pass_: int, rows: np.ndarray, recursion: bool, bpq_bspb_exp: np.ndarray, reusing_blocks: list):
    buffer = B_key.copy()
    if pass_ != 0:
    ...
    # if np.size(b_key[i:i+k[pass_]]) == k[pass_]:# блоки фиксированной длины для данного прохода
        if i + k[pass_] > np.size(B_key):
            binary_search(A_key[i:i+k[pass_]], B_key[i:i+k[pass_]], recursion, bpq_bspb_exp, reusing_blocks[pass_], i, np.size(B_key))
        else:
            binary_search(A_key[i:i+k[pass_]], B_key[i:i+k[pass_]], recursion, bpq_bspb_exp, reusing_blocks[pass_], i, i+k[pass_])
    if pass_ != 0:
        unshuffle_array(A_key, rows[pass_ - 1])
        unshuffle_array(B_key, rows[pass_ - 1])
        if not np.array_equal(B_key, buffer):
            Cascade(A_key, B_key, k, pass_-1, rows, True, bpq_bspb_exp, reusing_blocks)
...
def experiment(n: int, Q: float, k: list, passes: int):
    ...
    for i in range(passes):
        Cascade(AliceKey, BobKey, k, i, rows, False, bpq_bspb_exp, reusing_blocks)
